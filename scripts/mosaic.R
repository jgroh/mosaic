...
size.correct.herbarium <- function(x, prior = "equal", scaling = "standard"){
  ...
  # if( !is.numeric(trait.data$blade.cleft)){
  #   
  #   b <- toupper(trait.data$blade.cleft)
  #   for(i in 1:nrow(trait.data)){
  #     
...
plot(herb.scores.st ~ log.min.dist, type = "n",
     ...
     ylab = "Scaled discriminant score", yaxt = "n", cex.lab = 1.2, xlim = c(0, 8))
axis(2, las = 2)
...
######## FIGURE 3B: Color Analysis ########

color.data <- read.csv("/Users/jeff/Projects/Aquilegia/raw_data/ALL.color.current.3Aug018.csv", stringsAsFactors = F)

color.data$log.rg <- with(color.data, log(red.mean / green.mean))

color.data$site <- factor(color.data$site, 
                    levels = levels(as.factor(color.data$site) )[order( tapply( 
                      color.data$log.rg, color.data$site, mean) ) ] )
colvec8.2 <- colvec8.1[c(1,2,3,4,5, 8, 6)]
stripchart(log.rg ~ site, data = color.data, vertical = TRUE, pch = 1, las = 2, 
           method = "jitter" , ylab = "Sepal red vs. green reflectance\n (log R/G)", 
           col = colvec8.2, xaxt = "n", yaxt = "n", lwd = 2, cex.axis = 1.2, 
           cex.lab = 1.2, ylim = c(-.05,.8))
axis(2, las = 2, cex.axis = 1.2)

legend('topleft', legend = levels(as.factor(color.data$site)), 
